...
public abstract class RunnerAbstract {
	...
	public RunnerAbstract(
		...
		start(
				getExperimentName(workload, vmAllocationPolicy, vmSelectionPolicy, parameter, ""+RandomConstants.CLOUDLET_UTILIZATION_SEED),
				outputFolder,
	...
	protected VmAllocationPolicy getVmAllocationPolicy(
		...
		if (vmAllocationPolicyName.equals("iqr")) {
			...
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationStaticThresholdV2(
					...
					vmSelectionPolicy,
					parameter);
		} else if (vmAllocationPolicyName.equals("sg")) {
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationStabilityGrouping(
					hostList,
					vmSelectionPolicy,
					parameter);
