...
public class Helper {
	...
	public static void printResults(
		...
		if (outputInCsv) {
			...
			data.append(experimentName + delimiter);
			data.append(parseExperimentName(experimentName, delimiter));
			data.append(String.format("%d", numberOfHosts) + delimiter);
			data.append(String.format("%d", numberOfVms) + delimiter);
			data.append(String.format("%.2f", totalSimulationTime) + delimiter);
			data.append(String.format("%.5f", energy) + delimiter);
			data.append(String.format("%d", numberOfMigrations) + delimiter);
			data.append(String.format("%.10f", sla) + delimiter);
			data.append(String.format("%.10f", slaTimePerActiveHost) + delimiter);
			data.append(String.format("%.10f", slaDegradationDueToMigration) + delimiter);
			data.append(String.format("%.10f", slaOverall) + delimiter);
			data.append(String.format("%.10f", slaAverage) + delimiter);
			...
			data.append(String.format("%.2f", meanTimeBeforeHostShutdown) + delimiter);
			data.append(String.format("%.2f", stDevTimeBeforeHostShutdown) + delimiter);
			data.append(String.format("%.2f", meanTimeBeforeVmMigration) + delimiter);
			data.append(String.format("%.2f", stDevTimeBeforeVmMigration) + delimiter);
			...
			if (datacenter.getVmAllocationPolicy() instanceof PowerVmAllocationPolicyMigrationAbstract) {
				...
				data.append(String.format("%.5f", executionTimeVmSelectionMean) + delimiter);
				data.append(String.format("%.5f", executionTimeVmSelectionStDev) + delimiter);
				data.append(String.format("%.5f", executionTimeHostSelectionMean) + delimiter);
				data.append(String.format("%.5f", executionTimeHostSelectionStDev) + delimiter);
				data.append(String.format("%.5f", executionTimeVmReallocationMean) + delimiter);
				data.append(String.format("%.5f", executionTimeVmReallocationStDev) + delimiter);
				data.append(String.format("%.5f", executionTimeTotalMean) + delimiter);
				data.append(String.format("%.5f", executionTimeTotalStDev) + delimiter);
	...
	public static String parseExperimentName(String name) {
		return parseExperimentName(name, ",");
	}
	 ...
	 * @return the string
	 */
	public static String parseExperimentName(String name, String delimiter) {
		Scanner scanner = new Scanner(name);
		StringBuilder csvName = new StringBuilder();
		scanner.useDelimiter("_");
		for (int i = 0; i < 4; i++) {
			if (scanner.hasNext()) {
				csvName.append(scanner.next() + delimiter);
			} else {
				csvName.append(delimiter);
			}
		}
		scanner.close();
	...
	public static void writeMetricHistory(
			...
			String outputPath) {
		writeMetricHistory(hosts, vmAllocationPolicy, outputPath, ",");
	}
	...
	public static void writeMetricHistory(
		...
		for (Host host : hosts) {
			...
			if (!vmAllocationPolicy.getTimeHistory().containsKey(host.getId())) {
				Log.print("WARNING!!!: Host #" + host.getId() + " does not appear to have metric history!\n");
				Log.print("Valid keys: " +vmAllocationPolicy.getTimeHistory().keySet() + "\n");
				continue;
			}
