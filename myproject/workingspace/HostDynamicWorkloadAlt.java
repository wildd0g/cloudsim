...
public class HostDynamicWorkloadAlt extends HostDynamicWorkload {
	...
	public double updateVmsProcessing(double currentTime) {
		...
		//execute super's updateVmsProcessing without log, save previous log state
		boolean previousLogEnableState = !Log.isDisabled();
		Log.disable();
		double smallerTime = super.updateVmsProcessing(currentTime);

		//restore log status if needed
		if (previousLogEnableState) {Log.enable();}
